            ...
            >
              {isActive && (
                <motion.div
                  ...
                  className="absolute left-0 top-0 bottom-0 w-[2.5px] bg-gradient-to-b from-transparent via-brass to-transparent"
                  transition={{ type: 'spring', stiffness: 380, damping: 35 }}
                />
              )}
              <item.icon className={cn('w-5 h-5 transition-transform duration-200 group-hover:scale-110 shrink-0 z-10', isActive ? 'text-primary' : 'text-muted-foreground group-hover:text-foreground')} />
              <AnimatePresence>
                {sidebarOpen && (
                  <motion.span
                    initial={{ opacity: 0 }}
