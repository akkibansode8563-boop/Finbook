    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <Card className="bg-slate-50/50 dark:bg-slate-100 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800/80 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-2xl font-black text-white">{kpis.activeLoansCount}</div>
        ...
        <Card className="bg-slate-50/50 dark:bg-slate-100 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800/80 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-2xl font-black text-white">{formatCurrency(kpis.totalOutstanding)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-slate-50/50 dark:bg-slate-100 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-white text-base">Recent Payments Collected</CardTitle>
            <CardDescription className="text-slate-500 dark:text-slate-400 text-xs">Latest repayments received at the collections counter.</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                {recentPayments.map((p) => (
                  <div key={p.id} className="flex justify-between items-center p-3 rounded-xl border border-slate-200 dark:border-slate-800/60 bg-slate-100/50 dark:bg-slate-50 dark:bg-slate-950/20 hover:bg-slate-100 dark:hover:bg-slate-800/10 transition-all text-xs">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <Link href={`/customers/${p.customer.id}`} className="font-bold text-white hover:text-violet-400 hover:underline block">
                          {p.customer.fullName}
                        </Link>
        ...
        <Card className="bg-slate-50/50 dark:bg-slate-100 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-white text-base">Recent Customers Registered</CardTitle>
            <CardDescription className="text-slate-500 dark:text-slate-400 text-xs">Latest customer KYC registrations added to the system.</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                {recentCustomers.map((c) => (
                  <div key={c.id} className="flex justify-between items-center p-3 rounded-xl border border-slate-200 dark:border-slate-800/60 bg-slate-100/50 dark:bg-slate-50 dark:bg-slate-950/20 hover:bg-slate-100 dark:hover:bg-slate-800/10 transition-all text-xs">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <Link href={`/customers/${c.id}`} className="font-bold text-white hover:text-violet-400 hover:underline block">
                          {c.fullName}
                        </Link>
